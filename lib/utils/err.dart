...
class Err extends StatefulWidget {
  ...
  Err({Key key, this.mL,this.m,this.p_title, this.bar_visibility}) : super(key : key);
...
class _ErrState extends State<Err> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: widget?.bar_visibility ? AppBar(
          flexibleSpace: Container(
//                height: 200,
              decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [
                    const Color(0xFF0c0e35),
        ...
        body: Center(
            child: Center(child: Column(
              ...
              children: <Widget>[
                Image.asset('assets/broken_wire.png', height: 80,),
                Padding(
                  padding: const EdgeInsets.only(left: 32.0,right: 32.0),
                  child: Text('${widget.m}' ?? 'Something went wrong', textAlign: TextAlign.center, style: TextStyle(color: Theme.of(context).primaryColor),),
                ),
                SizedBox(height: 8.0,),
                Card(
                  child: InkWell(
